...
import { addSocialLink, editSocialLink, deleteSocialLink, editLink, deleteLink } from '../actions';
...
const LinkForm = ({ username, bio, link, socialLinks: initialSocialLink = [] }: Props) => {
    ...
    const handleDeleteSocialLink = async (id: string) => {
        try {
            ...
            if (result?.success) {
                setUserSocialLinks(prev => prev.filter(link => link.id !== id));
    ...
    const profileForm = useForm<ProfileFormData>({
        ...
        defaultValues: {
            ...
            username: profile.username && profile.username.length >= 3 ? profile.username : "user123",
            bio: profile.bio || "",
        },
    });
    ...
    const onProfileSubmit = async (data: ProfileFormData) => {
        console.log("Submitting profile:", data); // Add this line
        try {
    ...
    const createLinkByUser = async (data: LinkFormData) => {
        ...
        // return in shape expected by the existing code (link?.data?.id)
        return { data: json };
    };
    ...
    const onEditLinkSubmit = async (data: LinkFormData) => {
        ...
        try {
            const result = await editLink(data, editingLinkId);
            if (result?.success && result?.data) {
                setLinks(prev => prev.map(l => l.id === editingLinkId ? { ...l, ...data } : l));
                toast.success(result.message);
            } else {
